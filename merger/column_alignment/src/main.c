...
int main(int argc, char **argv){
    ...
    char* path = argv[0];
    char* output = argv[1];
    int datasets_number = (int)strtol(argv[2], NULL, 10);
    int metrics_number = (int)strtol(argv[3], NULL, 10);
    if (argc < BASIC_ARGS + metrics_number*2) {
        printf("Illegal number of arguments, expected at least %d, received %d", BASIC_ARGS+metrics_number*2, argc);
        exit(-1);
    }
    ...
    for(int i=0; i<metrics_number; i++){
        known_negatives_percentages[i] = strtod(argv[BASIC_ARGS+i], NULL);
